...
heads = {'Lookahead','Random','Sweeping','Drosoph','Saccadic'};

if type == 1
    lookahead = 3;
    Aobstacle = zeros(2*m+1,2*n+1,5);
    time_to_detection_list = zeros(numRuns,5*size(obstacleGrid,3));
    num_successful_searches_list = zeros(1,5*size(obstacleGrid,3));
    ...
    for i = 1:size(obstacleGrid,3)         %For each obstacle configuration
        for j = 1:5                        %For each SearchID
            for k = 1:numRuns
                [time_to_detection,correct_identification] = experiment_sim(0.2,0.2,Aobstacle(:,:,i),j);
                time_to_detection_list(k,5*(i-1)+j)=time_to_detection;
                num_successful_searches_list(5*(i-1)+j) = num_successful_searches_list(i*j) + correct_identification;
                headers{5*(i-1)+j} = strcat(heads{j},num2str(i));
            end
        end
    end
...
if type == 2
    lookahead = 3;
    time_to_detection_list = zeros(numRuns,5*size(A,3));
    num_successful_searches_list = zeros(1,5*size(A,3));
    for i = 1:size(A,3)     %For each prior
        for j = 1:5         %For each SearchID
            for k = 1:numRuns
                [time_to_detection,correct_identification] = experiment_sim(0.2,0.2,A(:,:,i),j);
                time_to_detection_list(k,5*(i-1)+j)=time_to_detection;
                num_successful_searches_list(5*(i-1)+j) = num_successful_searches_list(5*(i-1)+j) + correct_identification;
                headers{5*(i-1)+j} = strcat(heads{j},num2str(i));
            end
        end
    end
...
if type == 3
    lookahead = 3;
    time_to_detection_list = zeros(numRuns,5*size(alphaList,2));
    num_successful_searches_list = zeros(1,5*size(alphaList,2));
...
if type == 4
    lookahead = 3;
    time_to_detection_list = zeros(numRuns,5*size(betaList,2));
    num_successful_searches_list = zeros(1,5*size(betaList,2));
