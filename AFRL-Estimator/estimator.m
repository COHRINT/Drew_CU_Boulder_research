classdef estimator < handle
    %ESTIMATOR Velocity, attitude, and altitude estimator for quadrotor
    ...
    properties
        ...
        n % number of Euler integrations to perform per time step
        
        var_accel % variance for accelerometers
        var_gyro  % variance for gyros
        var_alt   % variance for altimeter
        var_flow  % variance for flow sensor
        
        % sensor measurements and flags
        altimeter
        have_altimeter
    ...
    methods
        ...
        % constructor
        function obj = estimator(m, mu, std_dev_accel, std_dev_gyro, std_dev_alt, std_dev_flow, x0)
            ...
            if (nargin > 6)
                if (size(x0) == size(obj.xhat))
                    ...
                    disp(['estimator: expected x0 to be of size [' num2str(size(obj.xhat))...
                        '], got matrix of size [' num2str(size(x0)) ']. Initializing to zeros.'])
                end
            end
            ...
            obj.P = diag([.0001, .0001, .0001, .0001*(2*pi/190)^2, .0001*(2*pi/180)^2, .0001*(1*pi/180)^2, (0.0001)^2]);
            
            obj.var_accel = std_dev_accel^2;
            obj.var_gyro = std_dev_gyro^2;
            obj.var_alt = std_dev_alt^2;
            obj.var_flow = std_dev_flow^2;
            
            obj.S = diag([obj.var_gyro, obj.var_gyro, obj.var_gyro, obj.var_accel]);
            
            obj.Q = diag([.01, .01, .01, 0.01*(2*pi/180)^2, 0.01*(2*pi/180)^2, (0.05)^2, .001*(0.1)^2]);
